...
現在は60種をアプリ本体へ接続している。潮だまり・浅瀬・珊瑚の森・海の洞窟・深海の全5海域、通常種とレア種を含む全60種を4コマの透過スプライトへ置き換え済み。

- ミナミハコフグ案 → 既存の `shallow-puffer`（ミナミハコフグ）
- カーソルアンコウ案 → 既存の `deep-lantern`（カーソルアンコウ・レア）
...
- 水槽での基本表示は小 `60px`・中 `72px`・大 `88px`。魚種別の `scale` を重ね、大型種ほど差を広げる
- 全コマ右向き
- CSS の上下移動と、画像内のひれ・尾・発光の動きを分離する
- 深海生物はカーソルアンコウを粒度の目安にし、基本は1セルを仮想 `128 × 128px` へ正規化してから2倍化する。厳密な統一はせず、表示倍率が高いリュウグウノツカイは輪郭だけを一段細かくする
...
![カーソルアンコウの4コマ](./key-angler-strip.png)

- ファイル: `key-angler-strip.png`
- 枠: レア
- 配置: 深海
- モチーフ: 実在のアンコウを基礎に、提灯だけを白いマウスカーソル形にした創作種
- 動き: ゆっくり泳ぎ、3〜4 コマ目で提灯が控えめに明るくなる
- 推奨速度: 1 周 `1.2〜1.6s`
- 図鑑文案: 「カーソルの形の あかりを ゆらして、ことばの深海を ゆっくり泳ぐ。ふしぎなアンコウ。」
...
- イソスジエビは60px表示で細線が潰れないよう、胴の縞を太い4本へまとめ、脚と触角を少数の強い線へ整理した。
...
- ニシキテグリは青緑の体、3〜4本へ簡略化した太い橙色の迷路模様、大きな胸びれを識別点にし、アオブダイと輪郭・模様が重ならないようにした。
...
| 種ID | 表示名 | ファイル | 動き | 1コマ |
|---|---|---|---|---:|
| `cave-soldierfish` | アカマツカサ | `cave-soldierfish-strip.png` | 尾と胸びれ、最後に泡 | 280ms |
| `cave-sweeper` | リュウキュウハタンポ | `cave-sweeper-strip.png` | 尾と胸びれ、最後に泡 | 260ms |
| `cave-pineconefish` | マツカサウオ | `cave-pineconefish-strip.png` | 尾とひれ、顎の発光器、最後に泡 | 320ms |
| `cave-boxer-shrimp` | オトヒメエビ | `cave-boxer-shrimp-strip.png` | 長い触角とはさみ、最後に泡 | 320ms |
| `cave-moray` | トラウツボ | `cave-moray-strip.png` | 体をS字にくねらせ、最後に泡 | 340ms |
| `cave-featherstar` | ウミシダ | `cave-featherstar-strip.png` | 羽枝を開閉して潮になびく | 380ms |
| `cave-spiny-lobster` | イセエビ | `cave-spiny-lobster-strip.png` | 触角と歩脚、最後に泡 | 340ms |
| `cave-triton` | ホラガイ | `cave-triton-strip.png` | 足と触角を伸縮、最後に泡 | 400ms |
| `cave-flashlight-fish` | ヒカリキンメダイ | `cave-flashlight-fish-strip.png` | 眼下の発光器を明滅、最後に泡 | 300ms |
| `cave-bullhead-shark` | ネコザメ | `cave-bullhead-shark-strip.png` | 尾と胸びれ、最後に泡 | 360ms |
| `cave-escape-eel` | エスケープウナギ | `cave-escape-eel-strip.png` | 尾先のESCキーから小さく放電 | 320ms |
| `cave-alt-nautilus` | オルトオウムガイ | `cave-alt-nautilus-strip.png` | 触手を振り、殻中央のALTキーを押す | 360ms |

- 通常10種は暗い洞窟でも輪郭が読めるよう、アカマツカサの赤、ハタンポの銀、マツカサウオの黄、ウミシダの紫を明部として使う。
- 実在種の識別点は、アカマツカサの大きな眼、マツカサウオの黒縁の硬い鱗、オトヒメエビの赤白帯と長い触角、トラウツボの黄斑、イセエビの長い触角、ヒカリキンメダイの眼下の発光器、ネコザメの頭部の張り出しと2枚の背びれを優先した。
- ウミシダは主腕を8〜10本の太い束として残し、羽枝を細かな点列ではなく短いブロック状の房へまとめた。
- エスケープウナギは青灰色の細身の体とし、赤い `ESC` キーキャップを尻尾の先そのものとして一体化した。3〜4コマ目だけ尾先から小さく放電する。
- オルトオウムガイは赤褐色の縞がある殻の中央へ、`ALT` 刻印入りの大きなキーキャップを1つ組み込んだ。A・L・Tを別々のキーにはしない。
- 12種とも各セル外周に透明余白を残し、長い触角や羽枝を含めて隣接コマへはみ出さないよう整形した。

## 深海（通常種・レア種）

| 種ID | 表示名 | ファイル | 動き | 1コマ |
|---|---|---|---|---:|
| `deep-dumbo` | メンダコ | `deep-dumbo-strip.png` | 耳状のひれと傘膜を開閉 | 340ms |
| `deep-oarfish` | リュウグウノツカイ | `deep-oarfish-strip.png` | 長い体をS字に波打たせる | 320ms |
| `deep-anglerfish` | チョウチンアンコウ | `deep-anglerfish-strip.png` | 口と自然な発光器、最後に泡 | 360ms |
| `deep-isopod` | ダイオウグソクムシ | `deep-isopod-strip.png` | 歩脚を交互に動かす | 360ms |
| `deep-barreleye` | デメニギス | `deep-barreleye-strip.png` | 透明頭部内の緑色眼を回転 | 340ms |
| `deep-frilled-shark` | ラブカ | `deep-frilled-shark-strip.png` | 細長い体をうねらせる | 340ms |
| `deep-coelacanth` | シーラカンス | `deep-coelacanth-strip.png` | 肉質のひれと三葉尾を動かす | 360ms |
| `deep-sea-pig` | センジュナマコ | `deep-sea-pig-strip.png` | 長い管足でゆっくり歩く | 380ms |
| `deep-basket-star` | テヅルモヅル | `deep-basket-star-strip.png` | 太く簡略化した分枝を開閉 | 400ms |
| `deep-vampire-squid` | コウモリダコ | `deep-vampire-squid-strip.png` | 傘膜を反転し腕端を発光 | 360ms |
| `deep-lantern` | カーソルアンコウ | `key-angler-strip.png` | マウスカーソル形の提灯を発光 | 350ms |
| `deep-tab-jelly` | タブクラゲ | `deep-tab-jelly-strip.png` | TABキーと傘を押し縮める | 360ms |

- カーソルアンコウの魚体を深海全体のディテール量の目安にし、細かな鱗・斑点・枝・吸盤をむやみに増やさない。厳密な粒度統一は行わない。
- 初回の深海素材は仮想 `128 × 128px` を基準にした。再調整したリュウグウノツカイだけは、倍率 `1.8` で輪郭が粗く見えないよう、体の段差と青銀色の中間色を一段細かくした。
- メンダコは各コマで暗紺色の目を左右に離して2つ配置し、60px表示でも1つに結合して見えないようにした。
- カーソルアンコウは魚体と動きを維持し、提灯先端だけを白い矢印型マウスカーソルへ置き換えた。3〜4コマ目は輪郭を残したまま金色に発光する。
- タブクラゲは横長の `TAB` キーキャップを傘上部へ一体化し、72px表示でも3文字を判読できる大きさにした。
- 全48コマで外周8px以上を透明にし、細長い体や触手も隣接セルへはみ出さないよう整形した。
...
- デリートシャコの甲殻は青緑・紺・珊瑚色・金色の大きな色面へ整理し、細かな節境界を減らした。`DEL` は全4コマで一度ずつ判読できる大きさを保つ。
...
- カーソルアンコウ: 「既存の魚体・4コマ・粗いピクセルグリッドを維持し、提灯だけを白い矢印型マウスカーソルへ置き換える。3〜4コマ目は金色に発光」
...
- 深海11種: 「各実在種の識別点を太い輪郭と12〜16色の大きな色面だけで表す。1セル64px程度の仮想グリッド、細かな鱗・点描・ディザ・細い枝を禁止。タブクラゲだけ大きな `TAB` キーを傘へ一体化」
- 粒度再調整6種: 「イソスジエビ・デリートシャコ・ニシキテグリ・ウミシダは細線を大きな色面へ統合。リュウグウノツカイは体の輪郭だけを一段細かくする。メンダコは体を維持し、左右2つの目を全コマで明示」
- 共通制約: 「レア種ごとに指定したキー刻印以外の文字・UI・影・3D・過剰な発光を入れない。単色クロマキー背景、同じピクセルグリッド、1 セルに 1 匹」

通常素材は、組み込みの画像生成で作った高解像度原画からクロマキーを透過へ変換し、最近傍補間で `1024 × 256px` に書き出している。深海の初回生成では各セルを `128 × 128px` へ正規化してから2倍化した。再調整素材は生物ごとの読みやすさを優先し、生成原画の粗いグリッドを保ったまま各セルへ収め、リュウグウノツカイだけは小さめの輪郭段差を残した。いずれも最近傍補間を使い、原画の硬い輪郭を維持している。
