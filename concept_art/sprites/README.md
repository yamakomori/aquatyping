...
- 深海生物はカーソルアンコウを粒度の基準にし、1セルを仮想 `128 × 128px` へ正規化してから2倍化する。大型種でも1pxの細かな点描やディザを増やさない
...
- カーソルアンコウの魚体を深海全体の最大ディテール量として扱い、追加11種はそれ以上に細かな鱗・斑点・枝・吸盤を描き込まない。
- 高解像度の生成原画を4等分し、各コマを仮想 `128 × 128px` の作業セルへ最近傍補間で正規化してから、`256 × 256px` へ2倍化した。これにより大型種でも最低2px単位のドット塊を保つ。
...
通常素材は、組み込みの画像生成で作った高解像度原画からクロマキーを透過へ変換し、最近傍補間で `1024 × 256px` に書き出している。深海12種だけはドット粒度の上限を揃えるため、各セルを `128 × 128px` へ正規化してから `256 × 256px` へ2倍化した。いずれも最近傍補間を使い、原画の硬い輪郭を維持している。輪郭色とマゼンタ背景が干渉する場合は、魚体色を保護するため純青背景で再生成する。
